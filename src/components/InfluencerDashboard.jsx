...
export default function InfluencerDashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      if (!token) return
      const res = await fetch(`${baseUrl}/dashboard/influencer`, { headers: { Authorization: `Bearer ${token}` } })
      if (res.ok) setData(await res.json())
    }
    load()
  }, [token])

  if (!user) return <div className="text-white p-6">Please login as influencer.</div>
  if (user.role !== 'influencer') return <div className="text-white p-6">This dashboard is for influencers.</div>
  if (!data) return <div className="text-white p-6">Loading...</div>

  const { profile, bookings, messages, visits } = data
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 text-white">
      ...
      {!profile ? (
        <div className="bg-yellow-500/10 border border-yellow-400/30 text-yellow-200 p-4 rounded">
          Create your public profile to start receiving bookings.
        </div>
      ) : (
        <div className="grid md:grid-cols-3 gap-4">
          <Card title="Profile">
            <div className="text-slate-300">{profile.display_name} • {profile.city}</div>
            <div className="text-slate-400 text-sm">Visits: {visits}</div>
          </Card>
          <Card title="Bookings">
